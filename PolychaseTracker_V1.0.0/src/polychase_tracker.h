...
private:
    ...
    fdk::Hash geometryComposeState() override {
        ...
        return p ? p->geometryComposeState() : fdk::Hash();
    }
    fdk::Hash geometryEditVersionState() override {
        DD::Image::GeometryProviderI* p = geo_provider_in();
        return p ? p->geometryEditVersionState() : fdk::Hash();
    }
    bool geometryStateVaries() override {
        DD::Image::GeometryProviderI* p = geo_provider_in();
        return p ? p->geometryStateVaries() : false;
    }

    // --- GeometryProviderI: layer output (forward) ---
    usg::LayerRef buildGeometryLayer(
        bool appendTo = false,
        const fdk::TimeValueSet& sampleTimes = fdk::TimeValueSet()) override {
        DD::Image::GeometryProviderI* p = geo_provider_in();
        return p ? p->buildGeometryLayer(appendTo, sampleTimes) : usg::LayerRef();
    }
    ...
    void buildGeometryStage(usg::StageRef& stage,
                            ...
                            const fdk::TimeValueSet& sampleTimes = fdk::TimeValueSet()) override {
        DD::Image::GeometryProviderI* p = geo_provider_in();
        if (p) p->buildGeometryStage(stage, requestArgs, sampleTimes);
        else   stage.reset();
    }
#endif
