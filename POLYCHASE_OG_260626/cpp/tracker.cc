...
static std::optional<PnPResult> SolveFrame(const Database& database,
    ...
    for (int32_t flow_frame_id : cache.flow_frames_ids) {
        ...
        for (size_t i = 0; i < num_matches; i++) {
            ...
            // 2D occlusion mask: drop a correspondence if EITHER endpoint lies
            // in a masked region on its own frame (the source keypoint on the
            // flow frame, the target keypoint on the frame being solved). Empty
            // predicate => nothing is masked.
            if (is_masked &&
                (is_masked(flow_frame_id, kp) || is_masked(frame_id, tgt_kp))) {
                continue;
            }
    ...
    // --- user (helper) track constraints ---
    // Append, for each track with an observation on this frame, a FIXED world
    // point (the object-space anchor through the same fixed model matrix the flow
    // points use) paired with the track's measured pixel. These are deliberate
    // artist anchors, so they are NOT subject to the 2D mask and they carry a
    // higher PnP weight than the flow points.
    const size_t num_flow_points = cache.object_points_worldspace.size();
    std::vector<float> user_point_weights;
    for (const UserTrack& tr : user_tracks) {
        for (const UserTrackObservation& ob : tr.observations) {
            if (ob.frame_id != frame_id) {
                continue;
            }
            const Eigen::Vector3f world_point =
                model_matrix.block<3, 3>(0, 0) * tr.object_point +
                model_matrix.block<3, 1>(0, 3);
            cache.object_points_worldspace.push_back(world_point);
            cache.image_points.push_back(ob.image_point);
            user_point_weights.push_back(tr.weight * user_track_weight);
    ...
    // Build the PnP weight vector ONLY when user tracks contributed; otherwise
    // leave cache.weights empty so the flow-only path stays uniform-weighted,
    // identical to before. When present it must cover EVERY point: flow points at
    // weight 1, user points at their (already scaled) weight.
    if (!user_point_weights.empty()) {
        cache.weights.assign(num_flow_points, 1.0f);
        cache.weights.insert(cache.weights.end(), user_point_weights.begin(),
                             user_point_weights.end());
    }
    ...
    for (int32_t frame_id = opts.frame_from + dir;
        ...
        const std::optional<PnPResult> maybe_result =
            SolveFrame(database, camera_traj, model_matrix, frame_id,
                       accel_mesh, opts.pnp_opts, opts.is_masked,
                       opts.user_tracks, opts.user_track_weight, cache);
